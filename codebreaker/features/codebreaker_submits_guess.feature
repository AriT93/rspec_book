Feature: code-breaker submits guess
  ...
  code the mark includes on - sign

  Each position in the secret code can only be matched once.  For example, a
  guess of 1134 agains a secret code of 1234 would get 3 plus sings: one for
  each of the exact matches in the first, third and forth positions. The number
  match in the second position would be ignored.



  Scenario Outline: submit a gues
